...
function extractDateTimeFromISO(isoString: string): {
  date: string;
  time: string;
} {
...
async function findOrCreateCategory(
  ...
  const normalizedCategory =
    CategoryMapping[categoryName.toLowerCase()] || "Other";
  ...
  const { data: newCategory, error: createError } = await supabase
    ...
    .insert({
      ...
      description: `Events related to ${normalizedCategory.toLowerCase()}`,
    })
    .select("id")
    .single();
...
async function findOrCreateOrganizer(
  ...
  // Try to find existing organizer by business name and contact
  const { data: existingOrganizer, error: findError } = await supabase
    .from("organizers")
    .select("id")
    .eq("business_name", organizerInfo.name)
    .eq("contact_email", organizerInfo.contact || "")
    .maybeSingle();
